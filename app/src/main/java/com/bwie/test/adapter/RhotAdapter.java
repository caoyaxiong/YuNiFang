 ...
 */
public class RhotAdapter extends RecyclerView.Adapter<RhotHolder> {
    ...
    public RhotAdapter(Context context,AllInfoBean allInfoBean) {
    ...
    public void onBindViewHolder(RhotHolder holder, final int position) {
        holder.hot_tv.setText(allInfoBean.getData().getBestSellers().get(0).getGoodsList().get(position).getGoods_name());
        holder.now_price.setText("￥"+allInfoBean.getData().getBestSellers().get(0).getGoodsList().get(position).getShop_price()+"");
        holder.old_price.setText("￥"+allInfoBean.getData().getBestSellers().get(0).getGoodsList().get(position).getMarket_price()+"");
        holder.old_price.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG); //中划线
        Glide.with(context)
                .load(allInfoBean.getData().getBestSellers().get(0).getGoodsList().get(position).getGoods_img())
                .into(holder.hot_iv);
    ...
    public int getItemCount() {
        return allInfoBean.getData().getBestSellers().get(0).getGoodsList().size();
    }
}
